$(document).ready(function () {
    const getData = (options) => {
        axios
            ...
            .then(function (response) {
                ...
                fillPrice(response.data)
                if (options.url === `http://localhost:3000/properties`) {
    ...
    const fillPrice = (data) => {
        $('.item').each((index, element) => {
            data.forEach((el, i) => {
                if ($(element).find('.item--guesty').text() === el._id) {

                    console.log('match')
                    $(element).find('.per-night').text(el.price)
                }
            })
    ...
    const priceUpdater = () => {
        ...
        $('.item').each((index, element) => {
            ...
            $('.card-marker').each((i, el) => {
                if ($(element).find('.item--guesty').text() === $(el).find('.marker-guesty').text()) {
            ...
            if ($('#card-span').length === 1) {
                if ($('#card-span').attr('guesty') === $(element).find('.item--guesty').text()) {
    ...
    const priceController = (parsedProperties) => {
        ...
        parsedProperties.forEach((element, index) => {
            ...
            $(".item").each((i, el) => {
                if ($(el).find(".item--guesty").text() === element.id) {
                    //console.log(' same id')
                    let totalPrice = 0;
    ...
    const queryBuilder = (formData) => {
        ...
        $(".item").each((index, element) => {
            let guestyId = $(element).find(".item--guesty").text();
        ...
        if (date.startDate === `Check-in`) {
            options.url = `http://localhost:3000/properties`
        } else {
    ...
    // Show All properties or Filter Controller
    ...
    const showAllProperties = async () => {
        ...
        $(".item").each((index, element) => {
            if (!(destination === 'All') && (guests <= Number($(element).find('.srp-properties-card--guest-number').text()))) {
                console.log('destination: ', destination)
                if (destination === $(element).find('.item--city').text()) {
                    $(element).addClass('show');
                }
            } else if (guests <= Number($(element).find('.srp-properties-card--guest-number').text())) {
                $(element).addClass('show');
            }
        });
    ...
    const filterController = (data) => {
        if (data.availability.length !== 0) {
            ...
            $(".item").each((index, element) => {
                let currentId = $(element).find(".item--guesty").text();
    ...
    const loadData = () => {
        ...
        $(".item").each((index, element) => {
            if (!($(element).hasClass("hidden"))) {
                ...
                let cardTotalPrice = $('.item').eq(index).find('#totalPrice').text();
                let cardPerNight = $('.item').eq(index).find('.per-night').text();
                coordinates = [
                    Number($(element).find(".item--lang").text()),
                    Number($(element).find(".item--lat").text()),
                ]
                ...
                geoData.push({
                    ...
                    properties: {
                        type: "fnac",
                        guesty: $(element).find(".item--guesty").text(),
                        title: $(element).find(".nickname").text(),
                        //address: $(element).find(".location-field_address").text(),
                        imgURL: $(element).find(".item--img").attr("src"),
                        link: $(element).find(".item--link").attr("href"),
                        cardTotalPrice,
                        cardPerNight
                    },
                });
    ...
    let sliderController = (cardData) => {
        ...
        console.log(cardData.listingName)
        $('.map-slide_img').attr('src', cardData.cardImg);
        $('.map-slide_img').attr('srcset', cardData.cardImg);
    ...
    const renderMap = () => {
        ...
        let map = new mapboxgl.Map({
            container: "map",
            style: "mapbox://styles/urbanstayapi/clpuh5mul00wv01qm5r56emny",
        ...
        function createPopUp(currentFeature) {
            ...
            let cardImg = currentFeature.properties.imgURL
            cardData = {
                listingName,
                cardTotalPrice,
                cardPerNight,
        ...
        map.on("load", function () {
            ...
            map.on("click", "unclustered-point", function (e) {
                ...
                if ($(window).width() < 480) {
                    ...
                    $('.item').each((index, element) => {
                        if ($(element).find('.item--guesty').text() === e.features[0].properties.guesty) {
 ...
 */
                ...
                let cardImg = e.features[0].properties.imgURL
                cardData = {
                    listingName,
                    cardTotalPrice,
                    cardPerNight,
    ...
    $(".home-hero_search-icon").on("click", () => {
        localStorage.setItem("filter", "");
